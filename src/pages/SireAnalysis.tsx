...
const conceptionColor = (rate: number) => {
  if (rate >= 95) return 'hsl(142, 69%, 58%)';
  if (rate >= 80) return 'hsl(40, 63%, 49%)';
  return 'hsl(0, 86%, 71%)';
};

const conceptionBarColor = (rate: number) => {
  if (rate >= 95) return 'hsl(142, 69%, 58%)';
  if (rate >= 88) return 'hsl(100, 50%, 50%)';
  if (rate >= 80) return 'hsl(40, 63%, 49%)';
  return 'hsl(0, 86%, 71%)';
};
  ...
  return (
    <div className="bg-card border border-card-border rounded-md px-3 py-2 text-xs">
      ...
      {payload.map((p: any, i: number) => (
        <p key={i} style={{ color: p.color || 'hsl(40, 63%, 49%)' }}>{p.name}: {p.value}</p>
      ))}
    </div>
  );
};
...
export default function SireAnalysis() {
  ...
  const sorted = useMemo(() => {
    const s = [...sireStats];
    s.sort((a, b) => {
      const aVal = (a as any)[tableSortKey] ?? 0;
      const bVal = (b as any)[tableSortKey] ?? 0;
  ...
  const conceptionData = useMemo(() =>
    [...sireStats].sort((a, b) => b.ai_conception_rate - a.ai_conception_rate)
      .map(s => ({ name: s.sire, value: s.ai_conception_rate })),
    [sireStats]);

  const gestationData = useMemo(() =>
    [...sireStats].filter(s => s.avg_gestation_days > 0)
      .sort((a, b) => a.avg_gestation_days - b.avg_gestation_days)
      .map(s => ({ name: s.sire, value: s.avg_gestation_days })),
    [sireStats]);

  const bullPctData = useMemo(() => {
    if (!records) return [];
    // Only sires with 30+ calves of known sex
    ...
    return Array.from(bySire.entries())
      ...
      .map(([name, v]) => ({ name, value: Math.round((v.bull / v.total) * 1000) / 10 }))
  ...
  const topSire = useMemo(() => {
    ...
    return eligible.length > 0
      ? eligible.reduce((best, s) => s.ai_conception_rate > best.ai_conception_rate ? s : best, eligible[0])
      : null;
  }, [sireStats]);

  const mostUsed = useMemo(() => {
    if (sireStats.length === 0) return null;
    return [...sireStats].sort((a, b) => b.total_calves - a.total_calves)[0];
  }, [sireStats]);
  ...
  return (
    <div className="space-y-6">
              ...
              <TableHeader>
                <TableRow className="bg-sidebar border-card-border hover:bg-sidebar">
                  {[
                    ...
                    { key: 'ai_conception_rate', label: 'AI Conception %' },
                    { key: 'avg_gestation_days', label: 'Avg Gestation (days)' },
                    { key: 'avg_calf_bw', label: 'Avg BW (lbs)' },
                    { key: 'calf_survival_rate', label: 'Survival %' },
                    { key: 'bull_calf_pct', label: 'Bull Calf %' },
                    { key: 'performance_badge', label: 'Performance' },
                  ].map(col => (
                    <TableHead
                      key={col.key}
              ...
              <TableBody>
                {sorted.map((s, i) => (
                  ...
                  >
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative w-16 h-3 rounded-full bg-muted overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(s.ai_conception_rate, 100)}%`,
                              backgroundColor: conceptionColor(s.ai_conception_rate),
                            }}
                          />
                        </div>
                    ...
                    <TableCell>
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${badgeStyle(s.performance_badge)}`}>
                        {s.performance_badge}
                      </span>
                    </TableCell>
                  </TableRow>
      ...
      {/* Charts */}
      <div className="space-y-6">
        {/* Chart 1: AI Conception Rate */}
        <Card className="bg-card border-card-border">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">AI Conception Rate by Sire</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={Math.max(conceptionData.length * 35, 200)}>
              <BarChart layout="vertical" data={conceptionData} margin={{ left: 100 }}>
                ...
                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                  {conceptionData.map((d, i) => (
                    <Cell key={i} fill={conceptionBarColor(d.value)} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        {/* Chart 2: Avg Gestation Days */}
        <Card className="bg-card border-card-border">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Avg Gestation Days by Sire</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={Math.max(gestationData.length * 35, 200)}>
              <BarChart layout="vertical" data={gestationData} margin={{ left: 100 }}>
                ...
                <YAxis dataKey="name" type="category" tick={{ fill: 'hsl(219, 23%, 53%)', fontSize: 11 }} width={95} />
                <Tooltip content={<CustomTooltip />} />
                <ReferenceLine x={herdAvgGestation} stroke="hsl(0, 86%, 71%)" strokeDasharray="5 5"
                  label={{ value: `Avg: ${herdAvgGestation}`, fill: 'hsl(0, 86%, 71%)', fontSize: 10 }} />
                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                  {gestationData.map((d, i) => (
                    <Cell key={i} fill={d.value > herdAvgGestation + 2 ? 'hsl(0, 86%, 71%)' : 'hsl(40, 63%, 49%)'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        {/* Chart 3: Bull Calf % */}
        <Card className="bg-card border-card-border">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium">Bull Calf % by Sire (30+ calves of known sex)</CardTitle></CardHeader>
          <CardContent>
            {bullPctData.length > 0 ? (
              <ResponsiveContainer width="100%" height={Math.max(bullPctData.length * 35, 200)}>
                <BarChart layout="vertical" data={bullPctData} margin={{ left: 100 }}>
                  ...
                  <YAxis dataKey="name" type="category" tick={{ fill: 'hsl(219, 23%, 53%)', fontSize: 11 }} width={95} />
                  <Tooltip content={<CustomTooltip />} />
                  <ReferenceLine x={50} stroke="hsl(0, 86%, 71%)" strokeDasharray="5 5"
                    label={{ value: 'Expected 50/50', fill: 'hsl(0, 86%, 71%)', fontSize: 10 }} />
                  <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                    {bullPctData.map((d, i) => (
                      <Cell key={i} fill={d.value > 60 ? '#60a5fa' : 'hsl(142, 69%, 58%)'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ) : (
              <p className="text-muted-foreground text-sm py-8 text-center">Not enough data for bull calf % chart</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        {topSire && (
          <Card className="bg-card border-success/40">
            <CardContent className="p-5">
              <p className="text-xs text-success font-medium mb-1">🏆 Top Performer</p>
              <p className="text-xl font-bold text-foreground">{topSire.sire}</p>
              <p className="text-3xl font-bold text-success mt-1">{topSire.ai_conception_rate}%</p>
              <p className="text-xs text-muted-foreground mt-1">AI Conception Rate</p>
              <div className="mt-3 text-sm text-muted-foreground space-y-0.5">
                <p>{topSire.total_calves} calves · {topSire.calf_survival_rate}% survival · {topSire.avg_calf_bw} lbs avg BW</p>
                {topSire.total_calves < 50 && (
                  <p className="text-primary text-xs mt-2 italic">Small sample — monitor with additional breedings before drawing firm conclusions.</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className={`bg-card ${mostUsed.ai_conception_rate < 88 ? 'border-primary/40' : 'border-success/40'}`}>
            <CardContent className="p-5">
              {mostUsed.ai_conception_rate < 88 ? (
                <>
                  <p className="text-xs text-primary font-medium mb-1">⚠ Most Used · Below Average</p>
                  <p className="text-xl font-bold text-foreground">{mostUsed.sire}</p>
                  <p className="text-3xl font-bold text-destructive mt-1">{mostUsed.ai_conception_rate}%</p>
                  <p className="text-xs text-muted-foreground mt-1">AI Conception Rate</p>
                  <div className="mt-3 text-sm text-muted-foreground space-y-0.5">
                    <p>{mostUsed.total_calves} calves recorded</p>
                ...
                <>
                  <p className="text-xs text-success font-medium mb-1">✓ Most Used · Strong Performer</p>
                  <p className="text-xl font-bold text-foreground">{mostUsed.sire}</p>
                  <p className="text-3xl font-bold text-success mt-1">{mostUsed.ai_conception_rate}%</p>
                  <p className="text-xs text-muted-foreground mt-1">AI Conception Rate</p>
                  <div className="mt-3 text-sm text-muted-foreground space-y-0.5">
                    <p>{mostUsed.total_calves} calves · {mostUsed.calf_survival_rate}% survival</p>
                  </div>
                </>
              )}
            </CardContent>
